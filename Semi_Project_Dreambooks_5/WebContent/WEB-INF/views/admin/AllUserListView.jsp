		...
		<div id="main">
			...
			<div class="btn-div">
				<form action="<%=request.getContextPath()%>/admin/userFinder">
					...
					<button type="submit" class="btn btn-primary" name="searchKeyword" value="U">일반 회원</button>
					<button type="submit" class="btn btn-primary" name="searchKeyword" value="P">출판사 회원</button>
				</form>
			</div>
			<br>
			<table class="table table-hover">
				...
				<tbody>
					...
					<tr>
						<td><a
							href="<%=request.getContextPath()%>/admin/userInfo?userId=<%=u.getUserId()%>"><%=u.getUserId()%></a></td>
						<td><%=u.getUserName()%></td>
						<td><%=u.getEmail()%></td>
						<td><%=u.getPhone()%></td>
						<td><%=u.getUserRole().equals("A") ? "관리자" : (u.getUserRole().equals("P") ? "출판사" : "일반회원")%></td>
					</tr>
					<%
						}
						}
